...
If pagination is active, grab the next `page` of data and emit the result to the `result$` observable.

### `prevPage()`

If pagination is active, grab the previous `page` of data and emit the result to the `result$` observable.
...
The result$ observable will emit a new result whenever the ListService `update()`, `sort()` or `filter()` methods are called. The `result$` observable contains the current page of items, the active sorting options and properties to create pagination. The object that is returned from this observable looks like this:
